...
void CalculaWeyl(int muestreo, int nivel ){
  ...
  escupechord<<muestreo<<"_ChordsExtra.dat"<<std::ends;
  escupecondini<<muestreo<<"_condinixtra.dat"<<std::ends;
  escuperadial<<muestreo<<"_ChordsRadial.dat"<<std::ends;
  ...
  for(int i=-resolucion;i<resolucion;i++){
	...
	gsl_complex zhi;
	 ...
	 chordradialspace<<gsl_complex_abs(zhi)<<"\t"
			 <<gsl_complex_arg(zhi)<<"\t"
			 <<weylreal<<"\t"<<weylimag<<"\t"
